...
7. Verify eval-profile gateway, backend, model, memory, and auth settings before ingest.
   For every OpenClaw backend row, `memory.backend` must exactly match the backend under test (`oo-builtin` -> `"builtin"`, `oo-builtin-vector` -> `"builtin-vector"`, `oo-qmd` -> `"qmd"`). For `oo-builtin-vector`, also verify actual runtime `memory_search` tool results. `agents.defaults.memorySearch` alone is not sufficient.
...
- Never accept an OpenClaw row whose `backend_config.actual_memory_backend` does not exactly match the claimed backend. For `oo-builtin-vector`, additionally require session transcripts to show matching non-QMD runtime `memory_search` results.
...
- `backend_config.actual_memory_backend` exactly matches the claimed OpenClaw backend, with `backend_config.memory_backend_verified=true`
- for `oo-builtin-vector`, sampled session transcripts show runtime `memory_search` is not `provider=qmd`, `model=qmd`, `mode=search`
