...
7. Verify eval-profile gateway, backend, model, memory, and auth settings before ingest.
   For every OpenClaw backend row, `memory.backend` must exactly match the backend under test (`oc-builtin` -> `"builtin"`, `oc-builtin-vector` -> `"builtin-vector"`, `oo-qmd` -> `"qmd"`). For `oc-builtin-vector`, also verify actual runtime `memory_search` tool results. `agents.defaults.memorySearch` alone is not sufficient.
...
- Never accept an OpenClaw row whose `backend_config.actual_memory_backend` does not exactly match the claimed backend. For `oc-builtin-vector`, additionally require session transcripts to show matching non-QMD runtime `memory_search` results.
...
- for `oc-builtin-vector`, sampled session transcripts show runtime `memory_search` is not `provider=qmd`, `model=qmd`, `mode=search`
