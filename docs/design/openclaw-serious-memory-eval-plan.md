...
- Do not treat `agents.defaults.memorySearch` as proof that a run used builtin-vector retrieval. The active backend is controlled separately by `memory.backend`; it must exactly match the backend under test. Session transcripts must show matching runtime `memory_search` tool results, not `provider/model/backend=qmd`, for a publishable `oo-builtin-vector` row.
